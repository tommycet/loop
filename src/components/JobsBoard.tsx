...
export function JobsBoard({ refreshKey }: { refreshKey: number }) {
  ...
  const grouped = useMemo(
    () => COLUMNS.map((status) => ({ status, tasks: tasks.filter((t) => t.status === status) })),
    [tasks],
  );
    ...
    <section className="rounded-[2rem] border border-white/10 bg-[#0f0c16] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.35)]">
      <div className="mb-5 flex items-center justify-between">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-semibold tracking-tight text-white">Jobs board</h2>
        </div>
        <div className="text-[10px] uppercase tracking-[0.24em] text-white/40">{tasks.length} jobs</div>
      </div>
              ...
              {group.tasks.length === 0 ? (
                ...
                group.tasks.map((task) => (
                  <div key={task.id} className="rounded-2xl border border-white/8 bg-black/20 p-4">
                    ...
                    {task.description && (
                      <p className="mb-3 text-xs text-white/55">{task.description}</p>
                    )}
